    ...
    for source_country in countries:
        for fml_condition in fml_conditions:
            ...
            for split in os.listdir(base_dir):
                ...
                joblib.dump(clf.best_estimator_, os.path.join(save_dir, f"{source_country}_{fml_condition}_{split}_model.pkl"))
                with open(os.path.join(save_dir, f"{source_country}_{fml_condition}_{split}_features.txt"), "w") as f:
                    f.writelines(f"{ftr}\n" for ftr in selected)
